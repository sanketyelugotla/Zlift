...
// Helper to get partnerId filter based on authenticated user
const getPartnerIdFilter = (req) => {
  if (req.user && req.user.userType === "partner") {
    return { partnerId: req.user.id }
  } else if (req.user && req.user.role === "partner_manager" && req.user.partnerId) {
...
// Get Partner Dashboard Stats (for a logged-in Partner user)
const getPartnerDashboardStats = async (req, res) => {
  try {
    const partnerId = req.user.id // The ID of the authenticated Partner user
    ...
    // Successful orders for this partner
    const successfulOrders = await Order.countDocuments({
      partnerId,
      status: "delivered",
    })

    // Cancelled orders for this partner
    const cancelledOrders = await Order.countDocuments({
      partnerId,
      status: "cancelled",
    })

    // Total items in inventory for this partner
    const totalItems = await Product.countDocuments({ partnerId })

    // Today's revenue for this partner
    ...
    res.json({
      success: true,
      message: "Partner dashboard stats retrieved successfully",
      data: {
        totalOrders,
        successfulOrders,
        cancelledOrders,
...
const getRecentOrders = async (req, res) => {
  ...
  try {
    ...
    const filter = getPartnerIdFilter(req) // Use the helper function
...
const addInventoryItem = async (req, res) => {
  try {
    ...
    // Determine partnerId based on user type
    let partnerId
    if (req.user.userType === "partner") {
      partnerId = req.user.id
    } else if (req.user.role === "partner_manager" && req.user.partnerId) {
      partnerId = req.user.partnerId
    } else {
      return res.status(400).json({
        success: false,
        message: "Partner ID is required or user not authorized to add items",
      })
    }
...
const updateInventoryItem = async (req, res) => {
  try {
    ...
    // Determine partnerId based on user type for authorization
    let authorizedPartnerId
    if (req.user.userType === "partner") {
      authorizedPartnerId = req.user.id
    } else if (req.user.role === "partner_manager" && req.user.partnerId) {
      authorizedPartnerId = req.user.partnerId
    } else {
    ...
    // Find the product to ensure it belongs to the authorized partner
    const product = await Product.findOne({ _id: id, partnerId: authorizedPartnerId })
    if (!product) {
...
const deleteInventoryItem = async (req, res) => {
  try {
    ...
    const product = await Product.findOneAndDelete({ _id: id, partnerId: authorizedPartnerId })
...
module.exports = {
  ...
  getPartnerDashboardStats, // Exported new function
